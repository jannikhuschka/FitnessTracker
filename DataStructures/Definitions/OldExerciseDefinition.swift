...
struct OldExerciseDefinition: HashEqCod, Identifiable {
	...
	init(id: UUID = UUID(), name: String, baseWeight: Double, weightStep: Double, setCount: Int, repCount: Int, weightStage: Int, pauseMode: OldPauseMode = .fixedPauseDuration, pauseModeDuration: Int = 60, isSupersetMember: Bool = false) {
		self.id = id
		self.name = name
